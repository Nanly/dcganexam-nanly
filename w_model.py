...
class GAN(object):
    ...
    def _read_input(self,filename_queue):# du qu picture
        ...
        record = DataRecord()
        decode_image = tf.image.decode_jpeg(value, channels=3)

        cropped_image = tf.cast(tf.image.crop_to_bounding_box(decode_image,55, 35, self.crop_image_size, self.crop_image_size), tf.float32)
        decode_image_4d = tf.expand_dims(cropped_image, 0)
        resize_image = tf.image.resize_bilinear(decode_image_4d, [self.resized_image_size, self.resized_image_size])
        record.input_image = tf.squeeze(resize_image, squeeze_dims=[0])
        return record

    def _read_input_queue(self, filename_queue):#chuang jian dui lie
    ...
    def _cross_entroy_loss(self, logits, labels, name="x_entroy"):
        xentroy = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits, labels))
        tf.scalar_summary(name, xentroy)
        return xentroy
    ...
    def _get_optimizer(self, optimizer_name, learning_rate, optimizer_param):
        ...
        if optimizer_name == "Adam":
            ...
            raise ValueError("Unkown optimizer %s" % optimizer_name)

    def _train(self, loss_val, var_list, optimizer):
        grads = optimizer.compute_gradients(loss_val, var_list=var_list)
        for grad, var in grads:
            utils.add_gradients_summary(grad, var)
        return optimizer.apply_gradients(grads)

    def _setup_placeholer(self):
    ...
    def _gan_loss(self, logits_real, logits_fake, feature_real, feature_fake, use_features=False):
        discriminator_loss_real = self._cross_entroy_loss(logits_real, tf.ones_like(logits_real), name="disc_real_loss")
        discriminator_loss_fake = self._cross_entroy_loss(logits_fake, tf.zeros_like(logits_fake), name="disc_fake_loss")
        self.discriminator_loss = discriminator_loss_fake + discriminator_loss_real

        gen_loss_disc = self._cross_entroy_loss(logits_fake, tf.ones_like(logits_fake), name="gen_disc_loss")
        if use_features:
    ...
    def create_network(self, generator_dims, discriminator_dims, optimizer="Adam", learning_rate=2e-4, optimizer_param=0.9, improved_gan_loss=True):
        ...
        self.generator_variables = [v for v in train_variables if v.name.startswith("gengerator")]
    ...
    def visulize_model(self):
        ...
        image = utils.unprocess_image(images, 127.5, 127.5).astype(np.uint8)
        shape = [4, self.batch_size // 4]
        utils.save_imshow_grid(images, self.logs_dir, "generated.png", shape=shape)


class WasserstienGAN(GAN):
    def __init__(self, z_dim, crop_image_size, resize_image_size, batch_size, data_dir, clip_values=(-0.01, 0.01),critic_iterations=5):
        self.critic_iterations = critic_iterations
        self.clip_values = clip_values
        GAN.__init__(self, z_dim, crop_image_size, resize_image_size, batch_size, data_dir)
    ...
    def _generator(self, z, dims, train_phase, activation=tf.nn.relu, scope_name="generator"):
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            for index in range(N - 2):
                ...
                h_conv_t = utils.conv2d_transpose_strided(h, W, b, output_shape=deconv_shape)
                h_bn = utils.batch_norm(h_conv_t, dims[index + 1], train_phase, sacope="gen_bn%d" % index)
                h = activation(h_bn, name='h_%d' % index)
                utils.add_activation_summary(h)
    ...
    def _discriminator(self, input_images, dims, train_phase, activation=tf.nn.relu, scope_name="discriminator", scope_reuse=False):
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            for index in range(N - 2):
                ...
                b = tf.zeros([dims[index + 1]])
                h_conv = utils.conv2d_transpose_strided(h, W, b)
                if skip_bn:
                    h_bn = h_conv
                    skip_bn =False
                else:
            ...
            b = tf.zeros([dims[-1]])
            h_pred = utils.conv2d_transpose_strided(h, W_pred, b)
    ...
    def train_model(self, max_iterations):
        try:
            print("Training Wassertein GAN model....")
            clip_discriminator_var_op = [var.assign(tf.clip_by_value(var, self.clip_values[0], self.clip_values[1]))
            ...
            for itr in xrange(1, max_iterations):
                ...
                if itr % 200 == 0:
                    ...
                    print("Time: %g/tir, Step: %d, generator loss: %g, discriminator_loss: %g" % (duration, itr, g_loss_val, d_loss_val))
