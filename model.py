...
class DCGAN(object):
    def __init__(self, sess, input_height=108, input_width=108, crop=True,
                 ...
                 max_to_keep = 1,
                 input_fname_pattern= '*.jpg', checkpoint_dir='ckpts', sample_dir='samples', output_dir= './out', data_dir='data'):
        """
        Args:
            sess:TensorFlow session
            batch_size: The size of batch. Shoule be specified before training.
            y_dim: (optional) Dimension of dim for y. [None]
            z_dim: (optional) Dimension of dim foe z. [100]
            #
            gf_dim: (optional) Dimension of gen filters in first conv layer. [64]
            df_dim: )optional) Dimension of discrim filters in first conv layer. [64]
            gfc_dim: (optional) Dimension of gen units for fully connected layer. [1024]
            dfc_dim: (optional) Dimension of discrim units for fully connectes layer. [1024]
        ...
        if dataset_name == 'mnist':
            ...
            if len(self.data) == 0:
                raise  Exception(["[!] No data foun in '" + data_path + "'"])
            ...
            if len(self.data) < self.batch_size:
                raise Exception("[!] Entire dataset size is less than the congigured batch_size")
    ...
    def train(self, config):
        ...
        if config.G_img_sum:
            ...
            self.g_sum = merge_summary([self.z_sum, self.d_sum, self.d_loss_fake_sum, self.g_loss_sum])
        self.d_sum = merge_summary([
                         self.z_sum, self.d_sum,self.d_loss_real_sum, self.d_loss_sum])
        self.writer = SummaryWriter(os.path.join(self.output_dir, "logs"), self.sess.graph)
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                print("[%8d Epoch: [%2d/%2d][%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f ]" \
                      % (counter, epoch, config.epoch, idx, batch_idxs, time.time() - start_time, errD_fake+errD_real, errG))
                ...
                if np.mod(counter, config.ckpt_freq) == 0:
                    self.save(config.chekpoint_dir, counter)
    ...
    def discriminator(self, image, y=None, reuse=False):
        with tf.variable_scope("discriminator") as scope:
            ...
            if not self.y_dim:
                ...
                h1 = lrelu(self.d_bn1(conv2d(h0, self.df_dim*2, name='d_1_conv')))
    ...
    def generator(self, z, y=None):
        with tf.variable_scope("generator") as scope:
            if not self.y_dim:
                ...
                return tf.nn.sigmoid(deconv2d(h2,
    ...
    def load_mnist(self):
        ...
        trY = np.asanyarray(trY)
        teY = np.asanyarray(teY)
    ...
    def save(self, checkpoint_dir, step, filename='model', ckpt=True, frozen=False):
        ...
        if frozen:
            tf.train.write_graph(
                ...
                checkpoint_dir, '{}-{:06d}_frz.pb'.format(filename, step), as_text=False)
